(ns pl.tomaszgigiel.palestra.codewars.gps
  (:gen-class))
...
; too slow?
(defn gps1 [s x]
  (int (/ (* 3600 (apply max 0 (map-indexed #(slice1 %1 %2 x) x))) s)))

(defn delta [m xs]
  (if-some [b (fnext xs)] (delta (max m (- b (first xs))) (rest xs)) m))

(defn gps [s x]
  (int (/ (* 3600 (delta 0 x)) s)))
